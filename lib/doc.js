...
Document.prototype._processTemplate = function(template) {
    var pageTemplate = dot.template(template);
    ...
    return mapObject(this.files, function (filePath, file) {
        ...
        for (i = 0; i < file.comments.length; i++) {
            ...
            if (code !== null) {
                codeLineNo++;
                code = numberPadding.substring(0, padLength - codeLineNo.toString().length) + codeLineNo + numberPadding2 + code;
            }
